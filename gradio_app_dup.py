...
def bot(history, session_image):
    ...
    if not isinstance(response, dict):
        response = {"result": str(response), "image_path": None}
...
def build_gradio_app():
    def show_modal_streaming(image_path):
        yield (
            gr.update(visible=True),  # Show modal
            gr.update(visible=True, value="⏳ Generating report, please wait..."),  # Show loading
            gr.update(visible=False),  # Hide image
            gr.update(visible=False)  # Hide file download
        )

        if not image_path or not os.path.exists(image_path):
            yield (
                gr.update(visible=True),
                gr.update(visible=True, value="❌ No image found."),
                gr.update(visible=False),
                gr.update(visible=False)
            )
        else:
            pdf_path = generate_pdf_report(image_path)
            yield (
                gr.update(visible=True),
                gr.update(visible=False),  # Hide loading text
                gr.update(value=image_path, visible=True),  # 👈 Ensure image preview is shown
                gr.update(value=pdf_path, visible=True)  # Show download button
            )
    def hide_modal():
        return gr.update(visible=False)

    with gr.Blocks(title="WaferGPT",theme='CultriX/gradio-theme',
                   ) as demo:
        with gr.Row():
        ...
        with gr.Sidebar(open=False):
            username_display = gr.Markdown("👤 Welcome!", elem_id="username-display")
            gr.Button("🚪 Logout", link="/logout", variant="secondary")
        ...
        with gr.Row():
            ...
            with gr.Column(visible=False,scale=1) as gallery_view:
                ...
                with gr.Row():
                    ...
                    # Create modal using gradio-modal
                with Modal(visible=False) as report_modal:
                    with gr.Column(scale=1, min_width=300):
                        gr.Markdown("### 📄 Report Preview")
                        loading_message = gr.Markdown("", visible=False)  # 👈 Add this line
                        # gr.Markdown("### 📄 Report Preview")
                        preview_image = gr.Image(label="Selected Image", scale=1, height=200)
                        download_btn = gr.File(label="⬇️ Download PDF Report", type="filepath")
            ...
            generate_report_btn.click(
                show_modal_streaming,
                inputs=[session_image],
                outputs=[report_modal, loading_message, preview_image, download_btn]
            )
        ...
        def show_username(request: gr.Request):
            return f"👤 Welcome : {request.username}!" if request.username else "👤 Welcome!"

        demo.load(show_username, inputs=None, outputs=username_display)
    return demo
